...
public class UserServiceImpl implements UserService, UserDetailsService {
    ...
    public AppUser saveUser(RegisterUserDTO registerUserDTO) {
        ...
        if (userDb != null) throw new WebshopException(HttpStatus.UNPROCESSABLE_ENTITY, "/app/auth/register","Username already exists.");
        ...
        AppUser user = new AppUser(null, registerUserDTO.getName(), registerUserDTO.getUsername(), registerUserDTO.getPassword(), new ArrayList<>(Collections.singletonList(role)));
    ...
    public AppUser addRoleToUser(String username, String roleName, HttpServletRequest... request) {
        ...
        AppUser user = userRepository.findByUsername(username);
        if(user == null) throw new WebshopException(HttpStatus.NOT_FOUND, path, "User not found in the database.");
        Role role = roleRepository.findByRoleName(roleName);
        if(role == null) throw new WebshopException(HttpStatus.NOT_FOUND, path, "Role does not exist.");
    ...
    public void verifyRegistrationCredentials(String username, String password){
        ...
        if(username.length() < 2 || username.length()>20) throw new WebshopException(HttpStatus.BAD_REQUEST, "/api/auth/register", "Invalid username format. Expected length of 2 - 20 characters.");

        //Password
        //Minimum of 8 characters
        //At least one uppercase letter
        //At least one number
        if(password.length() < 8) throw new WebshopException(HttpStatus.BAD_REQUEST, "/api/auth/register", "Invalid password format. Minimum length - 8 characters.");
        if(!password.matches(".*[A-Z].*")) throw new WebshopException(HttpStatus.BAD_REQUEST, "/api/auth/register", "Invalid password format. Must contain at least one uppercase character.");
        if(!password.matches(".*[0-9].*")) throw new WebshopException(HttpStatus.BAD_REQUEST, "/api/auth/register", "Invalid password format. Must contain at least one digit.");
    }
